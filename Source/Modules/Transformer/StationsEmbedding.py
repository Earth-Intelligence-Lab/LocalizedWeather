...
class StationEmbed(nn.Module):
    ...
    def forward(self, madis_x: torch.Tensor, era5_x=None):
        ...
        all_emb = torch.cat([self.encoding_madis_layers[i](madis_x[:, :, :, i]) for i in range(self.madis_n_vars_i)],
                            dim=-1)
        # (n_batch, n_stations, hidden_dim * madis_n_vars_i)

        if self.era5_len is not None:
            era5_emb = torch.cat([self.encoding_era5_layers[i](era5_x[:, :, :, i]) for i in range(self.era5_n_vars)],
                                 dim=-1)
